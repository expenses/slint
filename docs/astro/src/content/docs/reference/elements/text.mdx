...
Text {
    ...
    transform-origin-x: self.width / 2;
    transform-origin-y: self.height / 2;
    font-size: 30pt;
}
```
</CodeSnippetMD>
...
### transform-origin-x
<SlintProperty propName="transform-origin-x" typeName="length" defaultValue='self.width / 2'/>

### transform-origin-y
<SlintProperty propName="transform-origin-y" typeName="length" defaultValue='self.height / 2'/>
