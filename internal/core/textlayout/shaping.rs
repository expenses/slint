...
fn with_dejavu_font<R>(mut callback: impl FnMut(&rustybuzz::Face<'_>) -> R) -> R {
    ...
    let dejavu_path: std::path::PathBuf =
        [env!("CARGO_MANIFEST_DIR"), "..", "common", "sharedfontique", "DejaVuSans.ttf"]
            .iter()
            .collect();
    let registered_fonts = collection.register_fonts(std::fs::read(&dejavu_path).unwrap().into(), None);
    ...
    let face =
        rustybuzz::Face::from_slice(font.blob.data(), font.index).expect("unable to parse dejavu font");
    callback(&face)
}
