...
struct Iter<'a, S: AsRef<[StyledTextParagraph]>> {
    parser: pulldown_cmark::Parser<'a>,
    list_state_stack: alloc::vec::Vec<Option<u64>>,
    style_stack: alloc::vec::Vec<(Style, usize)>,
    current_url: Option<pulldown_cmark::CowStr<'a>>,
    implicit_arg_index: usize,
    positioned_arg_index_max: usize,
    current_paragraph: Option<StyledTextParagraph>,
    args: &'a [S],
}

impl<'a, S: AsRef<[StyledTextParagraph]>> Iterator for Iter<'a, S> {
    type Item = Result<StyledTextParagraph, StyledTextError<'static>>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
         ...
         -> Result<(), StyledTextError<'static>> {
            ...
            while let Some(mut p) = string[pos..].find(['{', '}']) {
                ...
                let arg_index = if inner_arg_string.is_empty() {
                    let arg_index = self.implicit_arg_index;
                    self.implicit_arg_index += 1;
                    arg_index
                } else if let Ok(n) = inner_arg_string.parse::<u16>() {
                    let positioned_arg_index = n as usize;
                    self.positioned_arg_index_max =
                        self.positioned_arg_index_max.max(positioned_arg_index + 1);
                ...
                if let Some(arg) = self.args.get(arg_index) {
                    ...
                    return Err(StyledTextError::ArgumentOutOfRange(arg_index, self.args.len()));
                }
        ...
        for event in &mut self.parser {
            let indentation = self.list_state_stack.len().saturating_sub(1) as _;
            ...
            match event {
                ...
                pulldown_cmark::Event::End(pulldown_cmark::TagEnd::List(_)) => {
                    if self.list_state_stack.pop().is_none() {
                ...
                pulldown_cmark::Event::Start(tag) => {
                    let style = match tag {
                        ...
                        pulldown_cmark::Tag::Item => {
                            let old_paragraph = std::mem::replace(
                                ...
                                Some(begin_paragraph(
                                    indentation,
                                    Some(match self.list_state_stack.last().copied() {
                                        Some(Some(index)) => ListItemType::Ordered(index),
                                        _ => ListItemType::Unordered,
                                    }),
                                )),
                            );
                            if let Some(state) = self.list_state_stack.last_mut() {
                        ...
                        pulldown_cmark::Tag::List(index) => {
                            self.list_state_stack.push(index);
                            continue;
                        }
                        ...
                        pulldown_cmark::Tag::Link { dest_url, .. } => {
                            self.current_url = Some(dest_url);
                            Style::Link
                        }
                    ...
                    let current_paragraph = match self.current_paragraph.as_mut() {
                        Some(paragraph) => paragraph,
                        None => return Some(Err(StyledTextError::ParagraphNotStarted)),
                    };

                    self.style_stack.push((style, current_paragraph.text.len()));
                }
                ...
                pulldown_cmark::Event::End(_) => {
                    let (style, start) = if let Some(value) = self.style_stack.pop() {
                        value
                    } else {
                        return Some(Err(StyledTextError::Pop));
                    };
                                    ...
                                    match &*tag_type {
                                        "u" => {
                                            let current_paragraph =
                                                match self.current_paragraph.as_mut() {
                                                    Some(paragraph) => paragraph,
                                                    None => {
                                                        return Some(Err(
        ...
        if self.implicit_arg_index > 0 && self.positioned_arg_index_max > 0 {
            return Some(Err(StyledTextError::MixedPlaceholders));
        }

        if (self.positioned_arg_index_max == 0 && self.implicit_arg_index != self.args.len())
            || self.positioned_arg_index_max > self.args.len()
        {
            return Some(Err(StyledTextError::PlaceholderCountMismatch(
                self.implicit_arg_index.max(self.positioned_arg_index_max),
                self.args.len(),
            )));
        }
        ...
        self.current_paragraph.take().map(Ok)
    }
}
...
pub fn parse_interpolated<S: AsRef<[StyledTextParagraph]>>(
    ...
    Iter {
        parser,
        list_state_stack: Default::default(),
        style_stack: Default::default(),
        current_url: None,
        implicit_arg_index: 0,
        positioned_arg_index_max: 0,
